...
<enclosure url="{t['release_url']}" length="{t['filesize']}" type="audio/mpeg"/>
<guid isPermaLink="false">{t['release_url']}</guid>
...
def main():
    ...
    for item in items:
        ...
        assets.append((mp3_file, filesize))
        item["filesize"] = filesize

    # Write asset list for GitHub Actions
    with open("auf1_assets.txt", "w") as f:
        for file, size in assets:
            f.write(f"{file}|{size}\n")
