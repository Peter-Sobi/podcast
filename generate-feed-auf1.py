#!/usr/bin/env python3
# AUF1 – ROBUSTE VERSION MIT DATUM + CONTINUE-FIX
# - lädt ALLE neuen Episoden (kein STOP mehr)
# - überspringt nur bereits vorhandene Dateien
...
def main():
    ...
    # ALLE neuen Dateien laden (continue statt break)
    for entry in feed.entries:
        ...
        # UUID-Erkennung → nur überspringen, nicht stoppen
        if any(uuid_name in fn for fn in os.listdir(MEDIA_DIR)):
            log(f"SKIP: Already have → {uuid_name}")
            continue
