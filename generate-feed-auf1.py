...
# KONFIGURATION
# ---------------------------------------------------------
RSS_URL = "https://podcasts.apple.com/at/podcast/auf1-nachrichten/id1586277021"
...
def extract_audio_url(entry):
    ...
    if hasattr(entry, "enclosures") and entry.enclosures:
        for enc in entry.enclosures:
            if "audio" in enc.type or enc.href.endswith((".mp3", ".m4a")):
                return enc.href

    # 2) Apple liefert oft JSON im "summary"
    ...
    # 4) Fallback: Suche überall im Entry
...
def process_episodes():
    ...
    with open(ASSET_LIST, "w", encoding="utf-8") as out:
        for entry in entries:
            ...
            if not audio_url:
                print("WARNUNG: Keine Audio-URL gefunden für:", entry.title)
                continue
