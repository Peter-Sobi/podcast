...
def fallback_from_existing_files():
    """
    Wenn AUF1 keinen RSS liefert → Feed aus vorhandenen Dateien erzeugen.
    """
    log("[fallback] Erzeuge Feed aus vorhandenen Dateien im Ordner auf1_media")

    items = []
    for filename in os.listdir(MEDIA_DIR):
        if filename.endswith(".mp3"):
            # Dateiname enthält bereits DD.MM_
...
def main():
    ...
    if not rss_items:
        log("[auf1] FEHLER: RSS enthält keine Items – Fallback wird verwendet.")
        fallback_from_existing_files()
        return
