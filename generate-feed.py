...
def compress_mp3(input_file: Path, output_file: Path):
    """Compress MP3 to 96kbit/s using ffmpeg."""
    try:
        subprocess.run(
            [
                "ffmpeg",
        ...
        input_file.unlink()  # Original löschen
        output_file.rename(input_file)  # Komprimierte Datei übernehmen
    except Exception as e:
...
<rss version="2.0">
<channel>
<title>Apolut Kombinierter Podcast (Proxy, 96kbit)</title>
...
def main():
    ...
    for i, item in enumerate(items):
        filename = MEDIA_DIR / f"folge_{i+1}.mp3"
        ...
        compress_mp3(filename, filename.with_suffix(".tmp.mp3"))
