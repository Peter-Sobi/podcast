...
FEEDS = [
    "https://anchor.fm/s/1147f5660/podcast/rss",
    "https://feeds.acast.com/public/shows/apolut-standpunkte",
    "https://feeds.acast.com/public/shows/apolut-m-pathie",
]
...
BASE_MEDIA_URL = "https://peter-sobi.github.io/podcast/media/"
...
def fetch_rss(url: str) -> BeautifulSoup | None:
    log.info("Lade RSS-Feed: %s", url)
    try:
        resp = requests.get(url, timeout=20)
        resp.raise_for_status()
        return BeautifulSoup(resp.text, "xml")
    except Exception as e:
        log.error("RSS-Fehler: %s", e)
        return None
...
def robust_download(url: str, path: str) -> bool:
        ...
        try:
            with requests.get(url, stream=True, timeout=30) as r:
                ...
                if total_size > 0 and downloaded < total_size:
                    log.warning("Unvollständig (%d < %d) – neuer Versuch",
    ...
    log.error("Alle Versuche fehlgeschlagen: %s", url)
    return False
...
def download_episode(item: dict):
    ...
    if os.path.exists(path):
        log.info("Überspringe (bereits vorhanden): %s", filename)
        return path, pub_dt
...
def build_feed(files: list[tuple[str, datetime]]):
    log.info("Erzeuge neuen Apolut-Feed (%d Episoden)", len(files))

    rss = ET.Element("rss", version="2.0")
    channel = ET.SubElement(rss, "channel")

    ET.SubElement(channel, "title").text = "Apolut – Podcast (Kombiniert)"
    ET.SubElement(channel, "link").text = "https://apolut.net/"
    ET.SubElement(channel, "description").text = "Kombinierter Apolut-Podcastfeed"
    ...
    log.info("Feed gespeichert: %s", FEED_FILE)
...
def main():
    log.info("Starte kombinierten Apolut-Feed-Generator")
    ...
    if not all_items:
        log.error("Keine Items aus allen Feeds – Abbruch.")
        return
