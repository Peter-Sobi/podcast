...
def mp3_reachable(url: str | None) -> bool:
    if not url:
        return False
    try:
...
def parse_article(url: str) -> dict:
    html = safe_fetch(url)
    if not html:
        return {"title": "Fehler", "date": "", "link": url, "mp3": None}

    mp3 = extract_mp3(html)
    if not mp3 or not mp3_reachable(mp3):
        return {"title": "Kein MP3", "date": "", "link": url, "mp3": None}
...
<link>{BASE}</link>
<description>Kombinierter Feed aus allen Kategorien</description>
...
def main():
    ...
    latest = parsed[:20]  # mehr Folgen für besseren Überblick
