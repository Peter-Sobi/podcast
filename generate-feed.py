...
# ---------------------------------------------------------
# Kompression: Tagesdosis = 12 kbit Mono, Rest dynamisch
# ---------------------------------------------------------

def compress_mp3(input_file: Path, title: str):
    # Tagesdosis → immer 12 kbit Mono
    if "tagesdosis" in title.lower():
        temp = input_file.with_suffix(".12k.tmp.mp3")
        try:
            subprocess.run(
                [
                    "ffmpeg",
    ...
    # Alle anderen Episoden → dynamische Kompression
...
def main():
    ...
    for item in items:
        ...
        if not compress_mp3(mp3_file, item["title"]):
            continue
