...
def extract_slug_from_url(url: str) -> str:
    """
    Holt den Slug direkt aus der Apolut-URL.
    Beispiel:
    https://apolut.net/ukrainische-fluchtlinge-vom-asyl-zur-armee-von-rainer-rupp/
    → ukrainische-fluchtlinge-vom-asyl-zur-armee-von-rainer-rupp
    """
    try:
...
# ---------------------------------------------------------
# Ultra-Fallback 4.0 — nutzt URL-Slug + Titel-Slug
# ---------------------------------------------------------
...
def try_construct_mp3_urls(date: str, title: str, url_slug: str) -> list[str]:
    ...
    # Kurzslug ohne "-von-..."
    title_slug_short = title_slug
    if "-von-" in title_slug:
        title_slug_short = title_slug.split("-von-")[0]
...
def compress_mp3(input_file: Path):
    debug(f"Komprimiere Datei: {input_file}")
    ...
    if input_file.stat().st_size > 50 * 1024 * 1024:
        debug("Datei nach Kompression zu groß → gelöscht.")
        input_file.unlink()
        return False

    debug("Kompression erfolgreich.")
    return True
...
def parse_rss(xml_text: str) -> list[dict]:
    ...
    for item in channel.findall("item"):
        ...
        if not mp3:
            debug("HTML-Fallback erfolglos → Ultra-Fallback 4.0")
...
<rss version="2.0">
<channel>
<title>Apolut Kombinierter Podcast (Debug-Version)</title>
<link>https://apolut.net</link>
<description>Debug-Version mit Ultra-Fallback 4.0</description>
