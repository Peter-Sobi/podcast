...
def process_tagesdosis():
    html = fetch("https://apolut.net/category/tagesdosis/")
    if not html:
        return None

    links = re.findall(r"https://apolut\.net/tagesdosis-[^\"'/]+/", html)
    if not links:
        return None
    ...
    filename = MEDIA_DIR / f"{date.strftime('%Y-%m-%d')}_tagesdosis_{guid}.mp3"
...
def process_rss():
    ...
    for item in channel.findall("item"):
        ...
        filename = MEDIA_DIR / f"{dt.strftime('%Y-%m-%d')}_{guid}.mp3"
