...
def extract_mp3_from_page(url: str) -> str | None:
    """Fallback: MP3-URL direkt von der Episodenseite extrahieren."""
    ...
    # Apolut verwendet IMMER dieses Muster:
    # https://apolut.net/content/media/YYYY/MM/DATE-apolut.mp3
    m = re.search(r"https://apolut\.net/content/media/[0-9]{4}/[0-9]{2}/[^\"']+\.mp3", html)
    if m:
        print(f"Fallback: MP3 gefunden: {m.group(0)}")
        return m.group(0)
...
def parse_rss(xml_text: str) -> list[dict]:
    ...
    for item in channel.findall("item"):
        ...
        if not mp3:
            print(f"WARNUNG: Episode ohne MP3-URL im RSS → versuche Fallback: {title}")
...
<description>Stabiler Proxy-Feed mit komprimierten MP3-Dateien (mit Fallback)</description>
